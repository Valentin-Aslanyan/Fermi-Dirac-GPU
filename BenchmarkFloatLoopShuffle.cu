...
__global__ void d_j_calc_fshuff(float *d_params, float *E_j, float *B_vector, float *d_j_up, float *w, float *x)
{	
	...
	float d_j_up_temp=integrand*fermi*(1.0f-fermi_m);

	//Perform a standard reduction to integrate
    for (int d = d_datapoints>>1; d > 0; d >>= 1)
    {
	d_j_up_temp   += __shfl_down(d_j_up_temp,d);
    }

    if (node_num==0) 
    {	
	d_j_up[integral_num]=d_j_up_temp*lim;
    }
}
...
__global__ void d_k_calc_fshuff(float *d_params, float *E_i, float *C_vector,float *d_k_up, float *w, float *x)
{	
	//Calculate the integrand on each thread
	int node_num, integral_num;
	integral_num=threadIdx.x/d_datapoints;
	...
	float d_k_up_temp=int_w_w;

	//Series of reductions over the first integral
    for (int d = d_datapoints>>1; d > 0; d >>= 1)
    {
	d_k_up_temp   += __shfl_down(d_k_up_temp,d);
    }

    if (node_num==0) 
    {	
	d_k_up[integral_num]=d_k_up_temp*lim;
    }
...
__global__ void d_l_calc_fshuff(float *d_params, float *E_i, float *D_vector, float *d_l, float *d_le, float *w, float *x)
{	
	int node_num, integral_num;
	integral_num=threadIdx.x/d_datapoints;
	node_num=threadIdx.x % d_datapoints;
	...
	float d_l_temp=integrand;
	float d_le_temp=integrand*EGammaPrime;

    for (int d = d_datapoints>>1; d > 0; d >>= 1)
    {
	d_l_temp   += __shfl_down(d_l_temp,d);
	d_le_temp   += __shfl_down(d_le_temp,d);
    }
	__syncthreads();
    if (node_num==0) 
    {	
	d_l[integral_num]=d_l_temp*lim;
	d_le[integral_num]=d_le_temp*lim;
    }
}
...
void d_calculate_rates_fshuff(float *d_params,float *d_B_vector, float *d_C_vector, float *d_D_vector, float *d_E_j, float *d_E_i, float *d_j, float *d_k, float *d_l, float *d_x, float *d_w,float *h_params, float *h_j,float *h_k,float *h_l,float *h_w, float *h_x,  float T_r, int ionizations_number,int excitations_number,int h_datapoints,cudaStream_t *streams, int h_block_mult)
{	
	cudaMemcpy(d_params,h_params,sizeof(float)*2,cudaMemcpyHostToDevice);
	d_j_calc_fshuff<<<excitations_number/h_block_mult,h_datapoints*h_block_mult,0,streams[0]>>>(d_params,d_E_j,d_B_vector,d_j,d_w,d_x);
	d_l_calc_fshuff<<<ionizations_number/h_block_mult,h_datapoints*h_block_mult,0,streams[0]>>>(d_params,d_E_i,d_D_vector,d_l,d_l+ionizations_number,d_w,d_x);
	d_k_calc_fshuff<<<ionizations_number/h_block_mult,h_datapoints*h_block_mult,0,streams[1]>>>(d_params,d_E_i,d_C_vector,d_k,d_w,d_x);
...
float *d_params, *d_B_vector, *d_C_vector, *d_D_vector, *d_E_j, *d_E_i, *d_j, *d_k, *d_l, *d_x, *d_w;
...
d_calculate_rates_fshuff(d_params,d_B_vector, d_C_vector,d_D_vector, d_E_j, d_E_i, d_j, d_k, d_l, d_x,  d_w,h_params,h_j,h_k,h_l,h_w,h_x,T_r, ionizations_number,excitations_number,h_datapoints,streams, h_block_mult);
...
for(idx=0;idx<10;idx++)
	{
	h_j_gauss_integration_f(h_params[0],h_params[1],E_j[idx],B_vector+4*idx,h_datapoints,&h_j2, h_w, h_x);
	h_k_gauss_integration_f(h_params[0],h_params[1],E_i[idx],C_vector+5*idx,h_datapoints,&h_k2, h_w, h_x);
	h_l_gauss_integration_f(h_params[0],h_params[1],E_i[idx],T_r,D_vector+2*idx,h_datapoints,&h_l2, &h_le2, h_w, h_x);
	printf("%E %E %E %E %E %E %E %E\n",h_j[idx],h_j2,h_k[idx],h_k2,h_l[idx],h_l2,h_l[idx+ionizations_number],h_le2);
	}

h_j_gauss_integration_f(h_params[0],h_params[1],E_j[excitations_number-1],B_vector+4*(excitations_number-1),h_datapoints,&h_j2, h_w, h_x);
h_k_gauss_integration_f(h_params[0],h_params[1],E_i[ionizations_number-1],C_vector+5*(ionizations_number-1),h_datapoints,&h_k2, h_w, h_x);
h_l_gauss_integration_f(h_params[0],h_params[1],E_i[ionizations_number-1],T_r,D_vector+2*(ionizations_number-1),h_datapoints,&h_l2, &h_le2, h_w, h_x);
...
d_cleanup_f(d_params, d_B_vector, d_C_vector, d_E_j, d_E_i, d_j, d_k, d_l, d_x, d_w);
exit(0);
