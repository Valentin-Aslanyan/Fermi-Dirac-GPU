...
__global__ void d_k_calc_fshuffmem(float *d_params, float *E_i, float *C_vector,float *d_k_up, float *d_k_up_part, float *w, float *x)
{	
	//Calculate the integrand on each thread
	int node_num, integral_num;
	integral_num=threadIdx.x/d_datapoints;
	...
	float E1=x[integral_num]*E0prime;
	...
	float int_w_w=integrand*w[node_num]*w[integral_num]*E0prime;
	float d_k_up_temp=int_w_w*fermiE0*(1.0f-fermiE1)*(1.0f-fermiE0prime);
    ...
    if (node_num==0) 
    {	
	d_k_up_part[blockIdx.x+gridDim.x*integral_num]=d_k_up_temp;
    }
...
{	
	...
	d_k_calc_fshuffmem<<<ionizations_number/h_block_mult,h_datapoints*h_block_mult,0,streams[1]>>>(d_params,d_E_i,d_C_vector,d_k,d_k_part,d_w,d_x);
...
d_calculate_rates_fshuffmem(d_params,d_B_vector, d_C_vector,d_D_vector, d_E_j, d_E_i, d_j, d_k, d_k_part, d_l, d_x,  d_w,h_params,h_j,h_k,h_l,h_w,h_x,T_r, ionizations_number,excitations_number,h_datapoints,streams, h_block_mult);
